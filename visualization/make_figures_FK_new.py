...
import chardet
from hindbrain_structure_function.functional_type_prediction.FK_tools.load_cells2df import load_cells_predictor_pipeline
...
class make_figures_FK:
    ...
    def __init__(self, modalities=['pa'],
        ...
        self.selected_meshes = ["Retina", 'Midbrain', "Olfactory Bulb", "Forebrain", "Habenula", "Hindbrain",
                                "Hindbrain_line", "Spinal Cord", "raphe", 'eye1', 'eye2', 'cerebellar_neuropil',
                                'Rhombencephalon - Rhombomere 1',
                                'Rhombencephalon - Rhombomere 2', "cn1", "cn2",
        ...
        if load_kmeans_classes:
            for i, cell in all_cells.iterrows():
                if cell.imaging_modality == "photoactivation":
                    temp_path_pa = self.path_to_data / 'paGFP' / cell.cell_name / f"{cell.cell_name}_metadata_with_regressor.txt"
                    with open(temp_path_pa, 'r', encoding=get_encoding(temp_path_pa)) as f:
                        t = f.read()
                        all_cells.loc[i, 'kmeans_function'] = t.split('\n')[11].split(' ')[2].strip('"')
                        all_cells.loc[i, 'reliability'] = float(t.split('\n')[12].split(' ')[2].strip('"'))
                        all_cells.loc[i, 'direction_selectivity'] = float(t.split('\n')[13].split(' ')[2].strip('"'))
                        all_cells.loc[i, 'time_constant'] = int(t.split('\n')[14].split(' ')[2].strip('"'))

                elif cell.imaging_modality == "clem":
                    if cell.function == "neg_control":
                        all_cells.loc[i, 'kmeans_function'] = 'neg_control'
                    elif cell.function == "to_predict":
                        all_cells.loc[i, 'kmeans_function'] = 'to_predict'
                    else:
                        ...
                        with open(temp_path_clem, 'r') as f:
                            ...
                            all_cells.loc[i, 'kmeans_function'] = t.split('\n')[15].split(' ')[2].strip('"')
                            all_cells.loc[i, 'reliability'] = float(t.split('\n')[16].split(' ')[2].strip('"'))
                            all_cells.loc[i, 'direction_selectivity'] = float(
                                t.split('\n')[17].split(' ')[2].strip('"'))
                            all_cells.loc[i, 'time_constant'] = int(t.split('\n')[18].split(' ')[2].strip('"'))

                elif cell.imaging_modality == "EM":
                    all_cells.loc[i, 'kmeans_function'] = all_cells.loc[i, 'function']
            all_cells['function'] = all_cells['kmeans_function']
        if keywords != 'all':
        ...
        try:
            all_cells['cell_name'].to_csv(
                self.path_to_data.joinpath("make_figures_FK_output").joinpath("used_cells").joinpath(
                    f'{"_".join(self.keywords)}_{self.name_time.strftime("%Y-%m-%d_%H-%M-%S")}.txt'), index=False,
                header=None)
        except:
            pass
    ...
    def plot_projection(self, projection='z', show_brs=False, force_new_cell_list=False, rasterize=True,
        ...
        if plot_synapse_distribution and (np.unique(self.all_cells['imaging_modality']) == ['clem']).all():
            ...
            max_value = (np.ceil(np.max(
                [kde_values_post_ax0, kde_values_pre_ax0, kde_values_post_ax1, kde_values_pre_ax1]) * 1000)) / 1000
            ax_right.set_xlim(0, max_value)
            ax_top.set_ylim(0, max_value)
            ax_top.set_yticks([0, round(max_value / 2, 2), round(max_value, 2)],
                              [0, round(max_value / 2, 2), round(max_value, 2)])
            ax_right.set_xticks([0, round(max_value / 2, 2), round(max_value, 2)],
                                [0, round(max_value / 2, 2), round(max_value, 2)])
            ax_top.set_anchor('C')
            pos_main = ax.get_position().bounds
            ax_top.set_position((pos_main[0], pos_main[1] + pos_main[3] + 0.01, pos_main[2], pos_main[3] * 0.15))
        ...
        os.makedirs(
            self.path_to_data.joinpath("make_figures_FK_output").joinpath(projection_string[:30]).joinpath("pdf"),
            exist_ok=True)
        os.makedirs(
            self.path_to_data.joinpath("make_figures_FK_output").joinpath(projection_string[:30]).joinpath("png"),
            exist_ok=True)
        os.makedirs(
            self.path_to_data.joinpath("make_figures_FK_output").joinpath(projection_string[:30]).joinpath("svg"),
            exist_ok=True)

        # Save the figure in different formats at the specified resolution.
        fig.savefig(self.path_to_data.joinpath("make_figures_FK_output").joinpath(projection_string[:30]).joinpath(
            "pdf").joinpath(
            rf"{projection_string[:30]}{brkw}{'_'.join(self.keywords)}_{self.name_time.strftime('%Y-%m-%d_%H-%M-%S')}.pdf"),
            dpi=1200)
        fig.savefig(self.path_to_data.joinpath("make_figures_FK_output").joinpath(projection_string[:30]).joinpath(
            "png").joinpath(
            rf"{projection_string[:30]}{brkw}{'_'.join(self.keywords)}_{self.name_time.strftime('%Y-%m-%d_%H-%M-%S')}.png"),
            dpi=1200)
        fig.savefig(self.path_to_data.joinpath("make_figures_FK_output").joinpath(projection_string[:30]).joinpath(
            "svg").joinpath(
            rf"{projection_string[:30]}{brkw}{'_'.join(self.keywords)}_{self.name_time.strftime('%Y-%m-%d_%H-%M-%S')}.svg"),
            dpi=1200)
    ...
    def make_interactive(self, show_brs=True, force_new_cell_list=False, which_brs='whole_brain'):
        ...
        if show_brs:
            ...
            plotly.offline.plot(fig, filename=str(Path(
                self.path_to_data.joinpath("make_figures_FK_output").joinpath("html").joinpath(
                    f"interactive{brkw[-1]}{'_'.join(self.keywords)}_{self.name_time.strftime('%Y-%m-%d_%H-%M-%S')}.html"))),
    ...
    def plot_neurotransmitter(self, show_na=True):
        ...
        os.makedirs(self.path_to_data.joinpath("make_figures_FK_output").joinpath("neurotransmitter").joinpath("pdf"),
                    exist_ok=True)
        os.makedirs(self.path_to_data.joinpath("make_figures_FK_output").joinpath("neurotransmitter").joinpath("png"),
                    exist_ok=True)
        os.makedirs(self.path_to_data.joinpath("make_figures_FK_output").joinpath("neurotransmitter").joinpath("svg"),
                    exist_ok=True)
        ...
        fig.savefig(
            self.path_to_data.joinpath("make_figures_FK_output").joinpath("neurotransmitter").joinpath("pdf").joinpath(
                rf"neurotransmitter_stacked_bar_{'_'.join(self.keywords)}_{self.name_time.strftime('%Y-%m-%d_%H-%M-%S')}.pdf"),
            dpi=1200)
        fig.savefig(
            self.path_to_data.joinpath("make_figures_FK_output").joinpath("neurotransmitter").joinpath("png").joinpath(
                rf"neurotransmitter_stacked_bar_{'_'.join(self.keywords)}_{self.name_time.strftime('%Y-%m-%d_%H-%M-%S')}.png"),
            dpi=1200)
        fig.savefig(
            self.path_to_data.joinpath("make_figures_FK_output").joinpath("neurotransmitter").joinpath("svg").joinpath(
                rf"neurotransmitter_stacked_bar_{'_'.join(self.keywords)}_{self.name_time.strftime('%Y-%m-%d_%H-%M-%S')}.svg"),
            dpi=1200)
        ...
        fig.savefig(
            self.path_to_data.joinpath("make_figures_FK_output").joinpath("neurotransmitter").joinpath("pdf").joinpath(
                rf"neurotransmitter_simple_gray_{'_'.join(self.keywords)}_{self.name_time.strftime('%Y-%m-%d_%H-%M-%S')}.pdf"),
            dpi=1200)
        fig.savefig(
            self.path_to_data.joinpath("make_figures_FK_output").joinpath("neurotransmitter").joinpath("png").joinpath(
                rf"neurotransmitter_simple_gray_{'_'.join(self.keywords)}_{self.name_time.strftime('%Y-%m-%d_%H-%M-%S')}.png"),
            dpi=1200)
        fig.savefig(
            self.path_to_data.joinpath("make_figures_FK_output").joinpath("neurotransmitter").joinpath("svg").joinpath(
                rf"neurotransmitter_simple_gray_{'_'.join(self.keywords)}_{self.name_time.strftime('%Y-%m-%d_%H-%M-%S')}.svg"),
            dpi=1200)
