...
def find_branches(df,cell_name):
    def find_branch_terminal(current_cell,all_branch_segments, temp_terminal_list=None,current_length=0,all_length=0):
        ...
        for i, connection in all_branch_segments.loc[all_branch_segments['start_node'] == current_cell['end_node'], :].iterrows():
            all_length += connection['cable_length']
            if connection['end_type'] != 'end':
                temp_terminal_list,all_length = find_branch_terminal(connection,all_branch_segments, temp_terminal_list,current_length + connection['cable_length'],all_length)
            else:
                temp_terminal_list.append([connection['branch_id'],current_length + connection['cable_length']])
    ...
    all_branch_segments = pd.DataFrame(columns=['branch_id',"branch_type_nodes","branch_type_cl",'start_node', 'end_node', 'cable_length',"n_nodes","nodes",'connected2'])
    ...
    branch_id = 0
    for i,end in all_start_nodes.iterrows():
        ...
        temp_df = pd.DataFrame(columns=['branch_id','start_node', 'end_node', 'cable_length',"n_nodes","nodes",'connected2'])
        for mod_column in ["nodes",'connected2']:
    ...
    all_branch_segments.loc[all_branch_segments['n_nodes']<=(max_length*(1/5)),'branch_type_nodes'] = '0'
    all_branch_segments.loc[(all_branch_segments['n_nodes'] > (max_length*(1/5)))&
                            (all_branch_segments['n_nodes'] <= (max_length*(2/5))), 'branch_type_nodes'] = '1'
    all_branch_segments.loc[(all_branch_segments['n_nodes'] > (max_length * (2 / 5))) &
                            (all_branch_segments['n_nodes'] <= (max_length * (3 / 5))), 'branch_type_nodes'] = '2'
    all_branch_segments.loc[(all_branch_segments['n_nodes'] > (max_length * (3 / 5))) &
                            (all_branch_segments['n_nodes'] <= (max_length * (4 / 5))), 'branch_type_nodes'] = '3'
    all_branch_segments.loc[all_branch_segments['n_nodes'] > (max_length*(4/5)), 'branch_type_nodes'] = '4'
    ...
    for iq,segment_query in all_branch_segments.iterrows():
        connected_segments = []
        for it, segment_target in all_branch_segments.loc[all_branch_segments['branch_id']!=segment_query['branch_id'],:].iterrows():
    ...
    for i,branch in all_branch_segments.iterrows():
        temp_longest_neurite_in_branch, temp_total_branch_length = find_branch_terminal(branch, all_branch_segments)
        try:
            all_branch_segments.loc[i,'longest_neurite_in_branch'] = np.max(np.array(temp_longest_neurite_in_branch)[:,1])
        except:
    ...
    temp_longest_neurite_in_branch, temp_total_branch_length = find_branch_terminal(all_branch_segments.loc[(all_branch_segments['start_node']==all_branch_segments['start_node'].min())&
                                                                                                            (all_branch_segments['end_type']!='end'),:].iloc[0], all_branch_segments)
    end_longest_path = np.array(temp_longest_neurite_in_branch)[np.argmax(np.array(temp_longest_neurite_in_branch)[:,1]),0]
    item_iter = int(end_longest_path)
    all_branch_segments['main_path'] = False
    while True:
        all_branch_segments.loc[all_branch_segments['branch_id'] == item_iter, 'main_path'] = True
        start_node = all_branch_segments.loc[all_branch_segments['branch_id'] == item_iter, 'start_node'].iloc[0]


        if item_iter in all_branch_segments.loc[all_branch_segments['start_node']==all_branch_segments['start_node'].min(),'branch_id'].tolist():
            all_branch_segments.loc[all_branch_segments['branch_id'] == item_iter, 'main_path'] = True
            break
        item_iter = all_branch_segments.loc[all_branch_segments['end_node'] == start_node, 'branch_id'].iloc[0]

    quick_quantile = all_branch_segments.loc[(all_branch_segments['total_branch_length'] > 0),'total_branch_length']
