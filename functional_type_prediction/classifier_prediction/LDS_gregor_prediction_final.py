...
if __name__ == '__main__':
    ...
    features_train_clem, labels_train_clem, labels_imaging_modality_train_clem = features_train[labels_imaging_modality_train == 'clem', :], labels_train[labels_imaging_modality_train == 'clem'], \
    labels_imaging_modality_train[labels_imaging_modality_train == 'clem']
    ...
    turned_on_features3 = list(np.array(column_labels_train)[reduced_features_index]) + [
        'max_y_contra']  # + ['fraction_contra'] # + ['n_nodes_contra_hemisphere'] + ['n_nodes_contra_hemisphere_fraction']
    ...
    print(
        f'{np.sum([x in list(cell_selection.loc[cell_selection["predicted_reduced"] == "dynamic threshold", "cell_name"]) for x in manual_predicted_DTs])} of {len(manual_predicted_DTs)} manual predicted DTs predicted as DTs\n')
    ...
    print(Fore.RED + '\nAll' + Fore.BLACK)
    print(
        f'{np.sum([x in list(cell_selection.loc[cell_selection["predicted"] == "dynamic threshold", "cell_name"]) for x in manual_predicted_DTs])} of {len(manual_predicted_DTs)} manual predicted DTs predicted as DTs\n')
