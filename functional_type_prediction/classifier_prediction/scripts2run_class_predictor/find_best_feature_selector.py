...
class class_predictor:
    ...
    def load_brs(self, base_path, which_brs='raphe', as_volume=True):
        ...
        for file in os.listdir(base_path.joinpath("zbrain_regions").joinpath(which_brs)):
            try:
                self.meshes.append(
                    navis.read_mesh(base_path.joinpath("zbrain_regions").joinpath(which_brs).joinpath(file),
    ...
    def prepare_data_4_metric_calc(self, df, neg_control=True):
        ...
        if self.new_neurotransmitter:
            ...
            for i, cell in df.iterrows():
                if cell.imaging_modality == 'photoactivation':
                    if \
                    new_neurotransmitter.loc[new_neurotransmitter['Name'] == cell.cell_name, 'Neurotransmitter'].iloc[
                        0] is np.nan:
    ...
    def load_cells_features(self, file, with_neg_control=False, drop_neurotransmitter=False):
        ...
        self.labels_fk = self.labels_fk_with_to_predict[
            (self.labels_fk_with_to_predict != 'to_predict') & (self.labels_fk_with_to_predict != 'neg_control')]
        self.features_fk = self.features_fk_with_to_predict[
            (self.labels_fk_with_to_predict != 'to_predict') & (self.labels_fk_with_to_predict != 'neg_control')]
        self.labels_imaging_modality = self.labels_imaging_modality_with_to_predict[
            (self.labels_fk_with_to_predict != 'to_predict') & (self.labels_fk_with_to_predict != 'neg_control')]
        self.all_cells = self.all_cells_with_to_predict[
            (self.labels_fk_with_to_predict != 'to_predict') & (self.labels_fk_with_to_predict != 'neg_control')]
    ...
    def load_cells_df(self, kmeans_classes=True, new_neurotransmitter=True, modalities=['pa', 'clem'],
                      neg_control=True):
        """
        ...
        self.cells_with_to_predict = self.prepare_data_4_metric_calc(self.cells_with_to_predict,
                                                                     neg_control=neg_control)
        self.cells_with_to_predict = self.cells_with_to_predict.loc[self.cells_with_to_predict.cell_name.apply(
            lambda x: False if 'axon' in x else True), :]
        ...
        self.cells_with_to_predict['class'] = self.cells_with_to_predict.loc[:, ['function', 'morphology']].apply(
            lambda x: x['function'].replace(" ", "_") + "_" + x['morphology'] if x['function'] == 'integrator' else x[
                'function'].replace(" ", "_"), axis=1)

        self.cells = self.cells_with_to_predict.loc[(self.cells_with_to_predict['function'] != 'to_predict') & (
                    self.cells_with_to_predict['function'] != 'neg_control'), :]
    ...
    def select_features(self, train_mod: str, test_mod: str, plot=False, use_assessment_per_class=False,
        ...
        def find_optimum_PI(features_train, labels_train, features_test, labels_test, train_test_identical,
                            train_contains_test, train_mod, use_std_scale=False):
            """
        ...
        def find_optimum_custom(custom_scorer, features_train, labels_train, features_test, labels_test,
                                train_test_identical, train_contains_test, train_mod, use_std_scale=False):
            """
            ...
            if train_test_identical:
                ...
                for no_features in range(1, features_train.shape[1] + 1):
                    ...
                    try:
                        model = SelectFromModel(custom_scorer, max_features=no_features).fit(features_train,
                                                                                             labels_train)
                    except:
    ...
    def select_features_RFE(self, train_mod, test_mod, cv=False, estimator=None, scoring=None, cv_method=None,
                            save_features=False, cv_method_RFE='lpo'):
        mod2idx = {'all': np.full(len(self.pa_idx), True), 'pa': self.pa_idx, 'clem': self.clem_idx}
        """
        ...
        if cv_method is None:
            cv_method = [ShuffleSplit(n_splits=100, test_size=0.3, random_state=0), LeavePOut(p=1),
                         StratifiedKFold(n_splits=5), KFold(n_splits=5)]
        else:
            ...
            for estimator in all_estimator:
                ...
                for i in np.arange(1, self.features_fk.shape[1] + 1):
                    ...
                    acc = self.do_cv(method=cv_method_RFE,
                                     clf=LinearDiscriminantAnalysis(solver='lsqr', shrinkage='auto'), feature_type='fk',
                                     train_mod=train_mod, test_mod=test_mod,
                                     figure_label=str(estimator) + "_n" + str(i), fraction_across_classes=True,
                                     n_repeats=100, idx=selector.support_, plot=False)
                    acc_list.append(acc)
                plt.figure()
                plt.plot(acc_list)
                plt.axvline(np.nanargmax(acc_list), c='red', alpha=0.3)
                plt.text(np.nanargmax(acc_list) + 1, np.mean(plt.ylim()), f'n = {np.nanargmax(acc_list) + 1}',
                         fontsize=12, color='red', ha='left', va='bottom')
                plt.gca().set_xticks(np.arange(0, self.features_fk.shape[1], 3),
                                     np.arange(1, self.features_fk.shape[1] + 2, 3))
                plt.title(f"{str(estimator)}\nAccuracy {acc_list[np.nanargmax(acc_list)]}%", fontsize='small')
                selector = RFE(estimator, n_features_to_select=np.nanargmax(acc_list) + 1, step=1).fit(self.features_fk,
                                                                                                       self.labels_fk)
                ...
                le = [Patch(facecolor='white', edgecolor='white', label=x) for x in
                      np.array(self.column_labels)[selector.support_]]
                ...
                plt.show()
                selector = RFE(estimator, n_features_to_select=np.argmax(acc_list) + 1, step=1).fit(self.features_fk,
                                                                                                    self.labels_fk)
                self.select_train_mod = train_mod
                self.select_test_mod = test_mod
                self.select_method = str(estimator)
                self.confusion_matrices(LinearDiscriminantAnalysis(solver='lsqr', shrinkage='auto'), method='lpo',
            ...
            for estimator in all_estimator:
                for scoring in all_scoring:
                    for cv in cv_method:
                        ...
                        temp_str = '\n'.join([x.split('(')[0] for x in temp_str.split('\n')])
                        print(temp_str, '\n')
                        accuracy = self.do_cv(method='lpo',
    ...
    def do_cv(self, method: str, clf, feature_type, train_mod, test_mod, n_repeats=100, test_size=0.3, p=1, ax=None,
              figure_label='error:no figure label', spines_red=False,
        ...
        acronym_dict = {'dynamic_threshold': "DT", 'integrator_contralateral': "CI", 'integrator_ipsilateral': "II",
                        'motor_command': "MC",
                        'dynamic threshold': "DT", 'integrator contralateral': "CI", 'integrator ipsilateral': "II",
                        'motor command': "MC"}
        ...
        if check_test_equals_train:
            ...
            for train_index, test_index in splitter.split(features_train):
                clf_work = clone(clf)
                X_train, X_test, y_train, y_test = features_train[train_index], features_test[test_index], labels_train[
                    train_index], labels_test[test_index]
            ...
            if method == 'lpo':
                for train_index, test_index in splitter.split(features_train):
                    clf_work = clone(clf)
                    X_train, X_test, y_train, y_test = features_train[train_index], features_test, labels_train[
                        train_index], labels_test
                ...
                for train_indeces, test_indeces in zip(ss_train.split(features_train), ss_test.split(features_test)):
                    ...
                    X_train, X_test, y_train, y_test = features_train[train_index], features_test[test_index], \
                    labels_train[train_index], labels_test[test_index]
                    if check_duplicates(X_train, X_test):
                        pass
    ...
    def confusion_matrices(self, clf, method: str, n_repeats=100,
        ...
        for train_mod, loc_x in zip(['ALL', "CLEM", "PA"], range(3)):
            for test_mod, loc_y in zip(['ALL', "CLEM", "PA"], range(3)):
                ...
                self.do_cv(method, clf, feature_type, train_mod.lower(), test_mod.lower(),
                           ...
                           n_repeats=n_repeats, test_size=test_size, p=p, idx=idx)
    ...
    def calculate_verification_metrics(self, calculate_smat=False, with_kunst=True, calculate4recorded=False):
        ...
        if calculate_smat:
            self.smat_fish = calculate_zebrafish_nblast_matrix(self.cells_with_to_predict, path_to_data=self.path,
                                                               with_kunst=with_kunst, return_smat_obj=True, prune=False)
        ...
        z_score_dt = lambda x: abs(
            (x - np.mean(list(self.nblast_values_dt.score_2))) / np.std(list(self.nblast_values_dt.score_2)))
        z_score_ii = lambda x: abs(
            (x - np.mean(list(self.nblast_values_ii.score_2))) / np.std(list(self.nblast_values_ii.score_2)))
        z_score_ci = lambda x: abs(
            (x - np.mean(list(self.nblast_values_ci.score_2))) / np.std(list(self.nblast_values_ci.score_2)))
        z_score_mc = lambda x: abs(
            (x - np.mean(list(self.nblast_values_mc.score_2))) / np.std(list(self.nblast_values_mc.score_2)))
        ...
        print(
            f'{(self.nb_matches_cells_nc["score_1"] >= cutoff).sum()} of {self.nb_matches_cells_nc.shape[0]} neg_control cells pass NBlast general test.')

        subset_predict_cells = list(
            self.nb_matches_cells_predict.loc[self.nb_matches_cells_predict['score_1'] >= cutoff, 'id'])
    ...
    def predict_cells(self, train_modalities=['clem', 'photoactivation'], use_jon_priors=True, suffix='',
        ...
        # align the two dfs
        super_df = pd.merge(self.all_cells_with_to_predict,
                            self.cells_with_to_predict.loc[:, ['cell_name', 'metadata_path', 'comment', 'swc']],
                            on=['cell_name'], how='inner')

        # Select data based on train_modalities
        self.prediction_train_df = pd.merge(self.all_cells,
                                            self.cells.loc[:, ['cell_name', 'metadata_path', 'comment', 'swc']],
                                            on=['cell_name'], how='inner')
        self.prediction_train_df = self.prediction_train_df[
            self.prediction_train_df.imaging_modality.isin(train_modalities)]
        ...
        exclude_axon_bool = (
            self.all_cells_with_to_predict.cell_name.apply(lambda x: False if 'axon' in x else True)).to_numpy()
        exclude_train_bool = (
            ~self.all_cells_with_to_predict.cell_name.isin(self.prediction_train_df.cell_name)).to_numpy()
        exclude_not_to_predict = ((self.all_cells_with_to_predict.function == 'to_predict') | (
                    self.all_cells_with_to_predict.function == 'neg_control')).to_numpy()
        ...
        if use_jon_priors:
            priors = [self.real_cell_class_ratio_dict[x] for x in np.unique(
                self.prediction_train_labels)]  # Hindbrain,Rhombomere 1-3’: {INTs: 311, MCs: 206, DTs: 22}),
        else:
            priors = [
                len(self.prediction_train_labels[self.prediction_train_labels == x]) / len(self.prediction_train_labels)
                for x in np.unique(self.prediction_train_labels)]
        if not predict_recorded:
            clf = LinearDiscriminantAnalysis(solver='lsqr', shrinkage='auto', priors=priors)
        ...
        self.prediction_predict_df.loc[:,
        ['DT_proba_scaled', 'CI_proba_scaled', 'II_proba_scaled', 'MC_proba_scaled']] = clf.predict_proba(
            self.prediction_predict_features) * true_positive_flat
        predicted_int_temp = np.argmax(self.prediction_predict_df.loc[:,
                                       ['DT_proba_scaled', 'CI_proba_scaled', 'II_proba_scaled',
                                        'MC_proba_scaled']].to_numpy(), axis=1)
    ...
    def plot_neurons(self, modality: str, scaled: bool = True, output_filename: str = "test.html") -> None:
        ...
        sub_df = self.prediction_predict_df[
            self.prediction_predict_df['imaging_modality'] == modality].copy().sort_values(f'prediction{scaled_suffix}')
    ...
    def add_new_morphology_annotation(self):
        dt_annotation = pd.read_excel(
            self.path / 'prediction' / 'auxiliary_files' / 'dt_morphology_annotation_gregor.xlsx')
        morphology_in_features_dict = {
            'ipsilateral': np.unique(self.features_fk[self.all_cells['morphology_clone'] == 'ipsilateral', 0])[0],
        ...
        for i, cell in dt_annotation.iterrows():
            ...
            self.cells_with_to_predict.loc[self.cells_with_to_predict['cell_name'] == cell['cell_name'], "morphology"] = \
            cell['morphology']
            self.all_cells.loc[self.all_cells['cell_name'] == cell['cell_name'], "morphology_clone"] = cell[
                'morphology']
            self.all_cells_with_to_predict.loc[
                self.all_cells_with_to_predict['cell_name'] == cell['cell_name'], "morphology_clone"] = cell[
                'morphology']
        ...
        for morphology in morphology_in_features_dict.keys():
            self.features_fk[(self.all_cells['morphology_clone'] == morphology) &
                             ...
                             (self.all_cells['imaging_modality'] == 'clem'), 0] = morphology_in_features_dict[
                morphology]
            self.features_fk_with_to_predict[(self.all_cells_with_to_predict['morphology_clone'] == morphology) &
                                             ...
                                             (self.all_cells_with_to_predict['imaging_modality'] == 'clem'), 0] = \
            morphology_in_features_dict[morphology]
...
if __name__ == "__main__":
    ...
    # select features
    with_neurotransmitter.select_features_RFE('all', 'clem', cv=False, cv_method_RFE='ss')  # runs through all estimator
    ...
    without_neurotransmitter.select_features_RFE('all', 'clem', cv=False,
                                                 cv_method_RFE='ss')  # runs through all estimator
