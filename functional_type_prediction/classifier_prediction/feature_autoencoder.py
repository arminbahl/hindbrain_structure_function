...
if __name__ == "__main__":
    ...
    # Initialize variables to store cumulative accuracy and confusion matrices
    accuracy_list = []
    confusion_matrices = []
    ...
    # Perform 100 different 70:30 train-test splits
    for i in range(10):
        ...
        print(f"Label Mapping (integer to string): {label_mapping}")

        # Model Definition
        autoencoder = models.Sequential([
            layers.InputLayer(input_shape=(X_train.shape[1],)),
            layers.Dense(32, activation='relu'),  # Encoder layer
        ...
        autoencoder.fit(X_train, y_train_cat, epochs=50, batch_size=16, verbose=0, validation_data=(X_test, y_test_cat))

        # Predict classes for the test set
        y_pred_cat = autoencoder.predict(X_test)
        y_pred_int = np.argmax(y_pred_cat, axis=1)

        # Extend the true and predicted labels
        true_labels.extend(y_test_int)
        pred_labels.extend(y_pred_int)
    ...
    print("Normalized Confusion Matrix over 100 Splits:")
    print(cm)
    ...
    plt.title('Normalized Confusion Matrix over 100 Splits')
