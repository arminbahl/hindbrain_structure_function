...
if __name__ == '__main__':
    ...
    for i,cell in cells_em.iterrows():
        ...
        cells_em.loc[i, 'proba_test'] = eval(meta_data_list[15].split(' ')[2])
        cells_em.loc[i, 'proba_test_reduced'] = eval(meta_data_list[16].split(' ')[2])
        cells_em.loc[i, 'nov_OCSVM'] = eval(meta_data_list[17].split(' ')[2])
        cells_em.loc[i, 'nov_OCSVM_reduced'] = eval(meta_data_list[18].split(' ')[2])
        cells_em.loc[i, 'nov_IF'] = eval(meta_data_list[19].split(' ')[2])
        cells_em.loc[i, 'nov_IF_reduced'] = eval(meta_data_list[20].split(' ')[2])
        cells_em.loc[i, 'nov_LOF'] = eval(meta_data_list[21].split(' ')[2])
        cells_em.loc[i, 'nov_LOF_reduced'] = eval(meta_data_list[22].split(' ')[2])
    ...
    for presynaptic_cell in [x for x in cells_em.presynaptic.unique() if x != 'seed_cell']:
        ...
        for i,cell in temp_connectome_df.iterrows():
            ...
            if add_cell:
                ...
                soma_mesh_list.append(cell['soma_mesh'])
        ...
        ax.axis('off')
        fig.savefig(path_to_figure_dir / f'{presynaptic_cell}_predicted_connectome_z_projection.pdf', dpi=1200)
        ...
        ax.set_facecolor('white')
        ax.axis('off')
        fig.savefig(path_to_figure_dir / f'{presynaptic_cell}_predicted_connectome_y_projection.pdf', dpi=1200)
