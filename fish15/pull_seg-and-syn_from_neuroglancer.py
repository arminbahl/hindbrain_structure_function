...
df = pd.read_excel(
    "/Users/jonathanboulanger-weill/Dropbox (Harvard University)/hb_connectome/hindbrain_structure_function/clem_zfish1/all_cells/all_cells_current.xlsx"
)
num_cells = df.shape
...
for element_idx in range(0, num_cells[0]):
    ...
    try:
        ...
        problematic_axons.append((axon_id, e))
...
for element_idx in range(0, num_cells[0]):
    ...
    # Your code that might raise an exception
        ...
        # If an exception occurs, store the problematic value
        problematic_dendrites.append((dendrites_id, e))
...
for element_idx in range(0, num_cells[0]):
    ...
    if element_type=='cell':        
        ...
        output_position = output_position.apply(lambda x: [2 * x[0], 2 * x[1], x[2]])
        ...
        input_position = input_position.apply(lambda x: [2 * x[0], 2 * x[1], x[2]])
        ...
        output_synapses=client.materialize.live_query(synapse_table,
                                    datetime.datetime.now(datetime.timezone.utc),
                                    filter_equal_dict = {'pre_pt_root_id': axon_id})
